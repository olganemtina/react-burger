import { ILoginFormData, ILoginResponseData, IRegisterFormData, IResetFormData } from "../models/auth";
import { IRequestData, IResponseData } from "../models/fetch";
...
export const loginRequest = async (formData: ILoginFormData) => {
	...
	return await requestWithBody(requestData).then((result: IResponseData<{user: ILoginResponseData}>) => {
...
export const updateAccessTokenRequest = async () => {
	...
	return await requestWithBody(requestData).then((result: IResponseData<null>) => {
		setCookie("refreshToken", result.refreshToken);
		setCookie("accessToken", result.accessToken);
		return result;
	});
...
export const getUserRequest = async () => {
	...
	}).then(async (result: IResponseData<{user: ILoginResponseData}>) => {
		if (!result.success && result.message === "jwt expired") {
			await updateAccessTokenRequest();
			requestGet(getUserUrl, {
				"Content-Type": "application/json",
				Authorization: getCookie("accessToken") || "",
			}).then((result: IResponseData<{user: ILoginResponseData}>) => {
				return result;
			});
		} else {
			return result;
...
export const updateUserRequest = async (formData: IRegisterFormData) => {
	...
	return await requestWithBody(requestData).then(async (result: IResponseData<ILoginResponseData>) => {
		if (!result.success && result.message === "jwt expired") {
...
export const passwordResetStep1Request = async (formData: {email: string}) => {
