import { setOrderFailedAction, setOrderRequestAction, setOrderSuccessAction } from "../action-creators/order";
...
export const SET_ORDER_FAILED: "SET_INGREDIENTS_FAILED" = "SET_INGREDIENTS_FAILED";

export const setOrder: AppThunk = (ids) => (dispatch: AppDispatch) => {
	dispatch(setOrderRequestAction());
	setOrderData(ids)
		.then((response) => {
			dispatch(setOrderSuccessAction(response.name, response.order?.number));
		})
		.catch((err) => {
